...
from build_features_T5 import get_epoch_training_data, get_example_rarities
...
def load_and_prepare_data(task, diff_dir):
    ...
    train_set = train_set.add_column('difficulty', data['diff'])
    ...
    train_val_split = train_set.train_test_split(test_size=0.1, seed=random_seed)
...
def train(args, output_dir):
    ...
    tokenizer = GPT2Tokenizer.from_pretrained(best_model_dir, use_fast=False)  # Optional
    model.to(device)
