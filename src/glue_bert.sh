...
do 
    ...
    #    do 
    #        sbatch -p titanx-long --gres=gpu:1 --mem=64gb --output=logs/bert/$LOGDIR/bert-$TASK-$strat-$o-length.log --wrap="python -u -m models.glue_ddaclae --gpu 0 --data-dir $DATADIR --strategy $strat --use-length --ordering $o --num-epochs $NUMEPOCHS --cache-dir $CACHEDIR --competency $COMP --task $TASK --num-obs $NUMOBS --diff-dir $DIFFDIR"
    #        sbatch -p titanx-long --gres=gpu:1 --mem=64gb --output=logs/bert/$LOGDIR/bert-$TASK-$strat-$o-irt.log --wrap="python -u -m models.glue_ddaclae --gpu 0 --data-dir $DATADIR --strategy $strat --ordering $o --num-epochs $NUMEPOCHS --cache-dir $CACHEDIR --competency $COMP --task $TASK --num-obs $NUMOBS --diff-dir $DIFFDIR" 
    #    done 
    #done
