...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name in ["mrpc", "rte", "mnli", "qqp", "sst2", "qnli"]:
...
def evaluate_model(task_loaders, task_name, model, label_nums, device, split='eval'):
    ...
    val_data = task_loader[split]['loader'].dataset
...
def trainer(args, teacher_model, student_model, action_model, task_loaders, label_nums, task_name):
    ...
    train_data = task_loader['train']['loader'].dataset
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.teacher_epochs:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                out = teacher_model.forward(src=input_ids, task_name=task_name.lower(), mask=attention_mask,
    ...
    try:
        os.makedirs("test_outputs_glue_2")
    except:
        pass

    test_pred.to_csv("./test_outputs_glue_2/" + "teacher_" + task_name + '.tsv', sep='\t', index=False)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            out = student_model.forward(src=input_ids, task_name=task_name.lower(), mask=attention_mask,
                                        token_type_ids=token_type_ids)
            student_out, student_pooler = out[0], out[1]

            with torch.no_grad():
                out = teacher_model2.forward(src=input_ids, task_name=task_name.lower(), mask=attention_mask,
                                             token_type_ids=token_type_ids)
                teacher_out = out[0]
        ...
        if args.local_rank in [-1, 0]:
            ...
            if score > best_score:
                torch.save(student_model.state_dict(),
                           "./models_new/student_pkd_{}_{}.ckpt".format(args.task, args.seed))
                best_score = score
        ...
        if args.local_rank in [-1, 0]:
            for batch in held_dataloader:
                ...
                with torch.no_grad():
                    _, _, _, teacher_pooler_output = teacher_model(src=input_ids, \
                                                                   task_name=action.lower(), mask=attention_mask,
                                                                   token_type_ids=token_type_ids)
                ...
                out = teacher_model2.forward(
                    task_name=action.lower(),
                ...
                if args.use_comp_loss == False:
                    if action.lower() in ['sts-b']:
                        teacher_loss = 0.5 * nn.MSELoss()(teacher_out.view(-1), labels.view(-1)) + 0.5 * nn.MSELoss()(
                            teacher_out2.view(-1), labels.view(-1))
                    else:
                        teacher_loss = 0.5 * nn.CrossEntropyLoss()(teacher_out.view(-1, num_labels),
                                                                   labels.view(-1)) + 0.5 * nn.CrossEntropyLoss()(
                            teacher_out2.view(-1, num_labels), labels.view(-1))
                else:
                    if action.lower() in ['sts-b']:
                    ...
                    if action.lower() not in ['sts-b']:
                        teacher_out = nn.Softmax(-1)(teacher_out).gather(dim=1,
                                                                         index=labels.long().view(-1, 1)).squeeze()
                        teacher_out2 = nn.Softmax(-1)(teacher_out2).gather(dim=1,
                                                                           index=labels.long().view(-1, 1)).squeeze()
                    else:
            ...
            if score > best_score2:
                torch.save(teacher_model2.state_dict(),
                           "./models_new/teacher_meta_{}_{}_{}.ckpt".format(loss_type, args.task, args.seed))
                best_score2 = score

    student_model.load_state_dict(torch.load("./models_new/student_pkd_{}_{}.ckpt".format(args.task, args.seed)))
    teacher_model2.load_state_dict(
        torch.load("./models_new/teacher_meta_{}_{}_{}.ckpt".format(loss_type, args.task, args.seed)))
    ...
    if args.local_rank in [-1, 0]:
        for episode in range(args.num_episodes):
            ...
            held_dataset_main = task_loaders[task_name]['held']['loader'].dataset
            ...
            for step, batch_main in enumerate(held_dataloader_main):
                ...
                actions = action_model(student_pooler_output2)[0]
                action_ = Categorical(actions).sample()
                action = all_tasks[action_.cpu().numpy()]

                prev_action = trajectory[-1]

                # input_ids_main, attention_mask_main, token_type_ids_main, labels_main = batch_main[0], batch_main[1], \
                # batch_main[2], batch_main[3]


                input_ids_main = batch_main['input_ids']
                attention_mask_main = batch_main['attention_mask']
                token_type_ids_main = batch_main['token_type_ids']
                labels_main = batch_main['labels']
                ...
                if task_name.lower() not in ['sts-b']:
                    ...
                    if args.reward_function == 'real':
                        reward = (torch.abs(labels_main - teacher_out) - torch.abs(
                            labels_main - student_out3)).float().sum()
                    elif args.reward_function == 'binary':
                        reward = (torch.abs(labels_main - teacher_out) > torch.abs(
                            labels_main - student_out3)).float().sum()
                    else:
            ...
            for state in batch_states:
                if state.shape[1] == 1:
                    # Pad the [1, 1] tensor to [1, 4]
                    padded_state = F.pad(state, (0, 3), "constant", 0)
                else:
                    padded_state = state
                padded_states.append(padded_state)


            # batch_states_tensor = torch.cat(padded_states, dim=0)
            # total_rewards = torch.sum(torch.FloatTensor(batch_rewards)).item() / len(held_dataset_main)
            # all_rewards.append(total_rewards)
            # reward_tensor = torch.FloatTensor(
            #     discount_rewards(torch.FloatTensor(batch_rewards) / len(held_dataset_main))).to(device)
            # action_tensor = torch.cat(batch_actions, 0).unsqueeze(1)
            #
            # batch_states_tensor = torch.abs(batch_states_tensor) + 1e-8
            # # logprob = torch.log(action_model.forward(batch_states))
            # logprob = torch.log(batch_states_tensor)
            ...
            print("Student Result: {}, ".format(out2))
        ...
        test_pred.to_csv("./test_outputs_glue_2/" + "student_PKD_" + task_name + '.tsv', sep='\t', index=False)
        ...
        test_pred.to_csv("./test_outputs_glue_2/" + "teacher_meta_" + task_name + "_" + loss_type + '.tsv', sep='\t',
                         index=False)
        ...
        test_pred.to_csv("./test_outputs_glue_2/" + task_name + "_" + str(args.timestamp) + '.tsv', sep='\t', index=False)
