...
def train(args, output_dir):
    ...
    print(args.difficulty_measurer)
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch == 0 and args.training_scheduler in ['DDS_MAE','DDS_MAE_hard']:
            theta_hat, model_capacity_time = evaluate_and_estimate(model, dev_dataloader,
        ...
        total_model_capacity_time += time_model_e - time_model_s  # Add time for getting epoch training data
        ...
        if args.training_scheduler in ['DDS_MAE','DDS_MAE_hard']:
            ...
            training_stats.append({
                ...
                'Model Capacity Evaluation Time': total_model_capacity_time,
...
def run():
    ...
    parser.add_argument('--training_scheduler',
                        choices=['baseline', 'ordered', 'simple', 'DDS_MAE', 'linear', 'root', 'DDS_MAE_hard'],
                        default='simple')
    ...
    for task in GLUETASKS:
        # if task =='mnli':
        #     args.difficulty_measurer = 'sentence_length'
        #     args.task = task
        #     for scheduler in Scheduler:
        #
        #         args.task = task
        #         args.training_scheduler = scheduler
        #         output_dir = "./results_1/{}_{}_{}_debertaV3".format(args.task, args.difficulty_measurer, args.training_scheduler)
        ...
        for diff in Diffs:
            for scheduler in Scheduler:
                args.difficulty_measurer =diff
                args.task = task
                args.training_scheduler = scheduler
                output_dir = "./results_2/{}_{}_{}_debertaV3".format(args.task, args.difficulty_measurer, args.training_scheduler)
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
