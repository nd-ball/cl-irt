...
def get_epoch_training_data(ts, args, epoch, theta_hat=None, lower_offset=np.NINF, upper_offset=0):
    if args.training_scheduler == 'baseline':
        return ts
    if args.training_scheduler == 'DDS_MAE':
        assert theta_hat is not None and args.ordering == 'easiest'
    if args.training_scheduler == 'DDS_MAE_hard':
    ...
    if args.training_scheduler == 'ordered':
        return train_2
    elif args.training_scheduler == 'simple':
        ...
        return train
    elif args.training_scheduler == 'DDS_MAE':
        train_idx = [i for i in range(len(input_ids)) if
                     (theta_hat + lower_offset) <= train_2['difficulty'][i] <= (theta_hat + upper_offset)]
        ...
        return train
    elif args.training_scheduler == 'DDS_MAE_hard':
        ...
        num_train=max(args.min_train_length, num_train)
        train['input_ids'] = train_2['input_ids'][:num_train]
        train['attention_mask'] = train_2['attention_mask'][:num_train]
        train['token_type_ids'] = train_2['token_type_ids'][:num_train]
        train['labels'] = train_2['labels'][:num_train]
