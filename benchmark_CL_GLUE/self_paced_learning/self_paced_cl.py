...
dev_batch_size = 128
num_epochs = 10  # Increase the number of epochs to allow more training
max_length = 128
...
def calculate_belief_function(model, batch, device):
    # Placeholder function for belief function calculation
    # Replace with your specific belief function calculation logic
    input_ids = batch["input_ids"].to(device)
    attention_mask = batch["attention_mask"].to(device)
    labels = batch["label"].to(device)
    with torch.no_grad():
        ...
        logits = outputs.logits
        probabilities = torch.softmax(logits, dim=-1)
        belief_function = torch.mean(probabilities, dim=-1)  # Example: use mean probability as belief function
    return belief_function


def calculate_comprehensive_difficulty(model, batch, device, xi):
    confidence = calculate_confidence(model, batch, device)
    belief_function = calculate_belief_function(model, batch, device)
    comprehensive_difficulty = (1 - xi) * confidence + xi * belief_function
    return comprehensive_difficulty
...
for task in GLUE_TASKS:
    ...
    for model_checkpoint in models:
        ...
        time_s = time.time()
        xi = np.linspace(0, 1, num_epochs)  # Linearly increasing xi from 0 to 1 over epochs
        ...
        for epoch in range(num_epochs):
            ...
            # Calculate comprehensive difficulty scores for all samples
            all_difficulties = []
            for batch in tqdm(train_dataloader, desc="Calculating difficulties"):
                comprehensive_difficulty = calculate_comprehensive_difficulty(model, batch, device, xi[epoch])
                all_difficulties.extend(comprehensive_difficulty.cpu().numpy())

            # Sort samples by comprehensive difficulty
            sorted_indices = np.argsort(all_difficulties)
            ...
            for step, batch in enumerate(tqdm(train_dataloader)):
                ...
                # Weight the loss by comprehensive difficulty
                comprehensive_difficulty = calculate_comprehensive_difficulty(model, batch, device, xi[epoch])
                weighted_loss = loss * comprehensive_difficulty.mean()
