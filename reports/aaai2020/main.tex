...
Such models can be fit using variational inference methods, where the latent parameters are estimated via variational distributions \cite{natesan_bayesian_2016} (CITE Minka and Us). 
...
\begin{equation} 
p(y_{ij} = 1 \vert \theta_j, b_i) = \frac{1}{1 + e^{-(\theta_j - b_i)}}
\end{equation}

The probability that subject $j$ will answer item $i$ incorrectly is:
\begin{equation} 
p(y_{ij} = 0 | \theta_j, b_i) = 1 - p(y_{ij} = 1 \vert \theta_j, b_i)
\end{equation} 


In order to learn an IRT model, one must have a set of $I$ items $\{i_0, i_1, \dots, i_I\}$, a set of $J$ subjects $\{j_0, j_1, \dots, j_J\}$, and the binary graded responses $y_{ij}$ of the subjects to each of the items.
The likelihood of a dataset of response patterns $Y$ from multiple subjects to a set of items given the model parameters $\theta$ and $b$ is:
\begin{align} 
p(Y \vert \theta, b) &= \prod_{j=1}^J \prod_{i=1}^I p(Y_{ij}=y_{ij} \vert \theta_j, b_i)
\end{align} 
where $y_{ij} = 1$ if individual $j$ answers item $i$ correctly and $y_{ij} = 0$ if they do not.
