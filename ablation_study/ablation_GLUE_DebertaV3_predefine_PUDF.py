...
import evaluate
from build_features import get_example_rarities, get_epoch_training_data
...
def load_and_prepare_data(args):
    ...
    if args.difficulty_measurer == 'IRT_AC':
        train_diff_file = f'{args.diff_dir}/{args.task.lower()}1pl/best_parameters.json'
        with open(train_diff_file, 'r') as file:
            data = json.load(file)
        train_diff = train_raw.add_column('difficulty', data['diff'])
    elif args.difficulty_measurer == 'sentence_length':
        if args.task in ['sst2']:
            diff = [len(p) for p in train_raw['sentence']]
        elif args.task in ['mrpc', 'rte']:
            diff = [len(p1) + len(p2) for p1, p2 in zip(train_raw['sentence1'], train_raw['sentence2'])]
        elif args.task in ['qqp']:
            diff = [len(p1) + len(p2) for p1, p2 in zip(train_raw['question1'], train_raw['question2'])]
        elif args.task in ['qnli']:
            diff = [len(p1) + len(p2) for p1, p2 in zip(train_raw['question'], train_raw['sentence'])]
        elif args.task == 'mnli':
            diff = [len(p1) + len(p2) for p1, p2 in zip(train_raw['premise'], train_raw['hypothesis'])]
        else:
            raise ValueError(f"Task {args.task} not supported for difficulty calculation")

        train_diff = train_raw.add_column('difficulty', diff)


    elif args.difficulty_measurer == 'word_rarity':
        if args.task in ['sst2']:
            rarity = get_example_rarities(train_raw['sentence'])
        elif args.task in ['mrpc', 'rte']:
            rarity1 = get_example_rarities(train_raw['sentence1'])
            rarity2 = get_example_rarities(train_raw['sentence2'])
            rarity = [r1 + r2 for r1, r2 in zip(rarity1, rarity2)]
        elif args.task in ['qqp']:
            rarity1 = get_example_rarities(train_raw['question1'])
            rarity2 = get_example_rarities(train_raw['question2'])
            rarity = [r1 + r2 for r1, r2 in zip(rarity1, rarity2)]

        elif args.task in ['qnli']:
            rarity1 = get_example_rarities(train_raw['question'])
            rarity2 = get_example_rarities(train_raw['sentence'])
            rarity = [r1 + r2 for r1, r2 in zip(rarity1, rarity2)]
        elif args.task in ['mnli']:
            rarity1 = get_example_rarities(train_raw['premise'])
            rarity2 = get_example_rarities(train_raw['hypothesis'])
            rarity = [r1 + r2 for r1, r2 in zip(rarity1, rarity2)]
        else:
            raise ValueError(f"Task {args.task} not supported for difficulty calculation")

        train_diff = train_raw.add_column('difficulty', rarity)


    else:
        train_diff = train_raw
    ...
    train_val_split = train_diff.train_test_split(test_size=0.1, seed=random_seed)
    train_dataset = train_val_split['train']
    val_dataset = train_val_split['test']

    test_dataset = raw_datasets['validation_matched'] if args.task == 'mnli' else raw_datasets['validation']
...
def create_dataset(dataset, task, tokenizer, max_length, include_difficulty=True):
    ...
    tokenized_dataset = dataset.map(lambda examples: tokenize_function(examples, task, tokenizer, max_length),
                                    batched=True)

    # Define the columns based on whether difficulty is included
    ...
    # Extract data for TensorDataset
    data_tensors = [tokenized_dataset[column] for column in columns]

    return TensorDataset(*data_tensors)
...
def evaluate_and_estimate(model, dataloader, device, mode='eval'):
    ...
    if mode == 'eval':
        ...
        print(f"Validation Accuracy: {validation_accuracy:.4f}")
        return validation_accuracy, val_loss, theta_hat

# Function to scale the difficulty values to the range [-3, 3]
def scale_difficulty(values, min_val=-1, max_val=1):
    min_difficulty = values.min()
    max_difficulty = values.max()
    scaled_values = (values - min_difficulty) / (max_difficulty - min_difficulty) * (max_val - min_val) + min_val
    return scaled_values

def train(args, output_dir):
    print(f"\nTask: {args.task}")
    device = torch.device('cuda' if torch.cuda.is_available() and args.gpu >= 0 else 'cpu')
    print("Difficulty_measurer:",args.difficulty_measurer)
    print("Training_scheduler:",args.training_scheduler)
    train_dataset, dev_dataset, test_dataset = load_and_prepare_data(args)
    ...
    train_dataset = create_dataset(train_dataset, args.task, tokenizer, args.max_seq_len, include_difficulty=True)
    dev_dataset = create_dataset(dev_dataset, args.task, tokenizer, args.max_seq_len, include_difficulty=True)
    test_dataset = create_dataset(test_dataset, args.task, tokenizer, args.max_seq_len, include_difficulty=False)
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch == 0 and args.training_scheduler in ['DDS_MAE', 'DDS_MAE_hard']:
            theta_hat = evaluate_and_estimate(model, dev_dataloader,
                                            device, mode='estimate')

            if args.difficulty_measurer != "IRT_AC":
                print("Re-scale the difficulty")
                # Extract the tensors from the TensorDataset
                tensors = train_dataset.tensors
                difficulty_tensor = tensors[-1]  # Assuming difficulty is the last tensor in the dataset

                # Convert the difficulty tensor to a numpy array
                difficulty_np = difficulty_tensor.numpy()

                # Apply the scaling function
                scaled_difficulty_np = scale_difficulty(difficulty_np)

                # Convert back to tensor
                scaled_difficulty_tensor = torch.tensor(scaled_difficulty_np, dtype=difficulty_tensor.dtype)

                # Create a new TensorDataset with the updated difficulty tensor
                new_tensors = list(tensors[:-1]) + [scaled_difficulty_tensor]
                train_dataset = torch.utils.data.TensorDataset(*new_tensors)
        ...
        filtered_training_data = get_epoch_training_data(train_dataset, args, epoch,
                                                         theta_hat=cur_cap,
                                                         lower_offset=args.lower_bound,
                                                         upper_offset=args.upper_bound)
        ...
        if args.training_scheduler in ['DDS_MAE', 'DDS_MAE_hard']:
            dev_acc, val_loss, theta_hat = evaluate_and_estimate(model, dev_dataloader, device,
                                                                                      mode='eval_estimate')
            # Accumulate the time
            if theta_hat > prev_cap:
                cur_cap = theta_hat
            ...
            dev_acc, val_loss = evaluate_and_estimate(model, dev_dataloader, device,
                                                      mode='eval')
    ...
    if not os.path.exists(best_model_dir):
        raise FileNotFoundError(f"Checkpoint directory {best_model_dir} not found.")
    if not os.path.isfile(os.path.join(best_model_dir, 'pytorch_model.bin')):
        raise FileNotFoundError(f"Checkpoint file {best_model_dir}/pytorch_model.bin not found before loading.")
    ...
    final_stats_filename = f"{output_dir}/final_stats_{task}_Training:{train_time:.2f}s_{actual_epochs}epochs_{test_acc:.4f}.json"
...
def run():
    GLUETASKS = ['rte', 'mrpc',  'sst2', 'mnli', 'qnli', 'qqp']
    # Diffs = ['sentence_length', 'word_rarity']
    # Scheduler = ['linear', 'root']

    # Diffs = ['sentence_length']
    # Scheduler = ['linear', 'root']
    ...
    parser.add_argument('--difficulty_measurer',
                        choices=['baseline', 'sentence_length', 'word_rarity', 'IRT_AC'],
                        default='IRT_AC')

    parser.add_argument('--training_scheduler',
                        choices=['baseline', 'simple', 'DDS_MAE', 'linear', 'root', 'DDS_MAE_hard'],
                        default='DDS_MAE')
    parser.add_argument('--ordering', choices=['easiest', 'hardest', 'middleout'], default='easiest')

    parser.add_argument('--min-train-length', type=int, default=128)
    ...
    for task in GLUETASKS:
        args.task=task
        output_dir = "./ablation_results/{}_{}_{}_debertaV3_4".format(args.task, args.difficulty_measurer, args.training_scheduler)
